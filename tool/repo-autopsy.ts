...
async function ensureRepo(repoInput: string): Promise<{ path: string; owner: string; repo: string } | string> {
  ...
  return { path: repoPath, owner, repo }
}

export const clone = tool({
  description: "Clone/update a GitHub repo locally for deep analysis. Returns the local path.",
  args: {
    repo: tool.schema.string().describe("GitHub repo (owner/repo or URL)"),
  },
  async execute({ repo }) {
    try {
      const result = await ensureRepo(repo)
      if (typeof result === "string") return result

      // Get basic stats
      const fileCount = await $`find ${result.path} -type f -not -path '*/.git/*' | wc -l`.text()
      const languages = await $`find ${result.path} -type f -not -path '*/.git/*' | sed 's/.*\\.//' | sort | uniq -c | sort -rn | head -10`.text()

      return `✓ Repo ready at: ${result.path}
...
export const structure = tool({
  ...
  async execute({ repo, path = "", depth = 4 }) {
    ...
    try {
      ...
      const tree = await $`tree -L ${depth} --dirsfirst -I '.git|node_modules|__pycache__|.venv|dist|build|.next' ${targetPath} 2>/dev/null || find ${targetPath} -maxdepth ${depth} -not -path '*/.git/*' -not -path '*/node_modules/*' | head -200`.text()
      return tree.trim()
    } catch (e) {
      return `Failed: ${e}`
    }
...
export const search = tool({
  ...
  args: {
    ...
    fileGlob: tool.schema.string().optional().describe("File glob filter (e.g., '*.ts')"),
    context: tool.schema.number().optional().describe("Lines of context (default: 2)"),
    maxResults: tool.schema.number().optional().describe("Max results (default: 50)"),
  },
...
export const ast = tool({
  ...
  args: {
    repo: tool.schema.string().describe("GitHub repo (owner/repo or URL)"),
    pattern: tool.schema.string().describe("ast-grep pattern (e.g., 'function $NAME($$$ARGS) { $$$BODY }')"),
    lang: tool.schema.string().optional().describe("Language: ts, tsx, js, py, go, rust (default: auto)"),
  },
  async execute({ repo, pattern, lang }) {
    ...
    try {
      ...
      const output = await $`ast-grep --pattern ${pattern} ${langArg} ${result.path} 2>/dev/null | head -200`.text()
      return output.trim() || "No matches found"
    } catch (e) {
...
export const hotspots = tool({
  ...
  async execute({ repo }) {
    ...
    try {
      const churn = await $`git -C ${result.path} log --oneline --name-only --pretty=format: | sort | uniq -c | sort -rn | grep -v '^$' | head -15`.text()
      outputs.push(`## Most Changed Files (Git Churn)\n${churn.trim()}`)
    } catch {}

    // Largest files
    try {
      const largest = await $`fd -t f -E .git -E node_modules -E __pycache__ . ${result.path} --exec wc -l {} 2>/dev/null | sort -rn | head -15`.text()
      outputs.push(`## Largest Files (by lines)\n${largest.trim()}`)
    } catch {}

    // Files with most TODOs/FIXMEs
    try {
      const todos = await $`rg -c 'TODO|FIXME|HACK|XXX' ${result.path} --glob '!.git' 2>/dev/null | sort -t: -k2 -rn | head -10`.text()
      if (todos.trim()) {
    ...
    // Recent activity
    try {
      const recent = await $`git -C ${result.path} log --oneline -20`.text()
...
export const stats = tool({
  ...
  async execute({ repo }) {
    ...
    try {
      const stats = await $`tokei ${result.path} --exclude .git --exclude node_modules --exclude vendor --exclude __pycache__ 2>/dev/null`.text()
      return stats.trim()
    } catch (e) {
...
export const secrets = tool({
  ...
  async execute({ repo }) {
    ...
    try {
      ...
      const output = await $`gitleaks detect --source ${result.path} --no-banner -v 2>&1`.text().catch(e => e.stdout || e.message)
...
export const find = tool({
  ...
  args: {
    ...
    type: tool.schema.enum(["f", "d", "l", "x"]).optional().describe("Type: f=file, d=dir, l=symlink, x=executable"),
    extension: tool.schema.string().optional().describe("Filter by extension (e.g., 'ts')"),
  },
  async execute({ repo, pattern, type, extension }) {
    ...
    try {
      ...
      const output = await $`fd ${pattern} ${result.path} ${typeArg} ${extArg} -E .git -E node_modules 2>/dev/null | head -50`.text()
...
export const exports_map = tool({
  ...
  args: {
    repo: tool.schema.string().describe("GitHub repo (owner/repo or URL)"),
    entryPoint: tool.schema.string().optional().describe("Entry point to analyze (e.g., 'src/index.ts')"),
  },
  async execute({ repo, entryPoint }) {
    ...
    // Find all exports
    try {
      const namedExports = await $`rg "^export (const|function|class|type|interface|enum|let|var) " ${result.path} --glob '*.ts' --glob '*.tsx' --glob '*.js' -o -N 2>/dev/null | sort | uniq -c | sort -rn | head -30`.text()
      if (namedExports.trim()) {
    ...
    // Default exports
    try {
      const defaultExports = await $`rg "^export default" ${result.path} --glob '*.ts' --glob '*.tsx' --glob '*.js' -l 2>/dev/null | head -20`.text()
      if (defaultExports.trim()) {
    ...
    try {
      const reexports = await $`rg "^export \\* from|^export \\{[^}]+\\} from" ${result.path} --glob '*.ts' --glob '*.tsx' --glob '*.js' 2>/dev/null | head -30`.text()
      if (reexports.trim()) {
    ...
    if (entryPoint) {
      ...
      if (existsSync(entryPath)) {
        ...
        outputs.unshift(`## Entry Point: ${entryPoint}\n\`\`\`typescript\n${content.slice(0, 2000)}${content.length > 2000 ? "\n// ... truncated" : ""}\n\`\`\``)
      }
    }
...
export const blame = tool({
  ...
  async execute({ repo, path, startLine, endLine }) {
    ...
    try {
      ...
      const output = await $`git -C ${result.path} blame ${lineRange} --date=short ${path} 2>/dev/null | head -100`.text()
...
export const cleanup = tool({
  ...
  args: {
    repo: tool.schema.string().describe("GitHub repo (owner/repo or URL) or 'all' to clear everything"),
  },
  async execute({ repo }) {
    if (repo === "all") {
